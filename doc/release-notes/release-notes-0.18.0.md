Patentcoin Core version 0.18.0 is now available from:

  <https://patentcoincore.org/bin/patentcoin-core-0.18.0/>
...
`/Applications/Patentcoin-Qt` (on Mac) or `patentcoind`/`patentcoin-qt` (on
Linux).
...
Patentcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Patentcoin Core on unsupported systems.
...
10.10. Additionally, Patentcoin Core does not yet change appearance when
...
`contrib/init/patentcoind.service`, it has been changed to use
`/var/lib/patentcoind` as the data directory instead of
`~patentcoin/.patentcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/patentcoind` will
exist has enough space (check using `df -h /var/lib/patentcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/patentcoin/patentcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://patentcoincore.org/en/contact/) the Patentcoin Core
...
The systemd init file (`contrib/init/patentcoind.service`) has been changed
to use `/var/lib/patentcoind` as the data directory instead of
`~patentcoin/.patentcoin`. This change makes Patentcoin Core more consistent with
...
`datadir` option in `/etc/patentcoin/patentcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/patentcoin/patentcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/patentcoin/patentcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/patentcoin/patentcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/patentcoin/patentcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/patentcoin/patentcoin/blob/master/doc/patentcoin-conf.md)
  about the `patentcoin.conf` file describes how to use it to configure
  Patentcoin Core.

- A new document introduces Patentcoin Core's BIP174 [Partially-Signed
  Patentcoin Transactions
  (PSBT)](https://github.com/patentcoin/patentcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/patentcoin/patentcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Patentcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Patentcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/patentcoin/patentcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/patentcoin/patentcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Patentcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Patentcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/patentcoin/patentcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/patentcoin/patentcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart patentcoind with the `-deprecatedrpc=generate`
...
- A new `patentcoin-wallet` tool is now distributed alongside Patentcoin
...
This section describes planned changes to Patentcoin Core that may affect
other Patentcoin software and services.

- Since version 0.16.0, Patentcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Patentcoin Core 0.20 (expected about
  a year after 0.18), Patentcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Patentcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Patentcoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Patentcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Patentcoin Core
...
- A sub-project of Patentcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Patentcoin Core.  See their [project
  page](https://github.com/patentcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Patentcoin Core's own implementation, although entropy
  gathered by Patentcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Patentcoin Core a
...
- On macOS, Patentcoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `patentcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libpatentcoin_server.a` (jnewbery)
...
- #13926 [Tools] patentcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like patentcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `patentcoin-qt -version` output formatting (hebasto)
...
- #11625 Add PatentcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libpatentcoin_server` (laanwj)
...
- #15154 Configure: patentcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/patentcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Patentcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libpatentcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in patentcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Patentcoin Core (Christewart)
...
- #12246 Bugfix: Only run patentcoin-tx tests when patentcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_patentcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_patentcoin` (MarcoFalke)
...
- #14186 patentcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_patentcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update patentcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/patentcoin-conf.md (hebasto)
...
- #15354 Add missing `patentcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct patentcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support patentcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/patentcoin/).
